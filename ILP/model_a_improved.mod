...
execute
{
	for(var i=1; i<=n; i++){
	  	...
	  	var error = false;
	  	if(length[i] > t){
	  		 writeln ( "Error: input data inegrity length[i]>t (timeSlots) for task " + i );	 
	  	}
	  	
	  	if(length[i] > max_deliver[i]){
	  		 writeln ( "Error(infeasible): input data inegrity length[i]>max_deliver[i] for task " + i );	 
	  	}
...
//>>>>>>>>>>>>>>>>
maximize sum(i in N)(sum(j in (min_deliver[i]-length[i]+1)..(max_deliver[i]-length[i]+1))x_ij[i,j] * profit[i]);  
//<<<<<<<<<<<<<<<<
...
subject to {
    ...
    We replace this constriant to cut solution space given the input assumptions.
    */
    
    forall(i in N)
    sum(j in (min_deliver[i]-length[i]+1)..(max_deliver[i]-length[i]+1) )x_ij[i,j] <= 1; 
    
    forall(i in N)
    (sum(j in (max_deliver[i]-length[i]+2)..t)x_ij[i,j]) == 0; 
    
    forall(i in N)
    (sum(j in 1..(min_deliver[i]-length[i]))x_ij[i,j]) == 0;
